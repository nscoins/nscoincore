...
void MasternodeList::updateMyMasternodeInfo(QString strAlias, QString strAddr, CMasternode* pmn)
{
    ...
    if (pmn) {
        ...
        switch (pmn->Level())
        {
            case 1: mnLevelText = getMnName(1); if (roi1 > 1) tLuck = ((masternodeCoins/COIN) / roi1)*100; break;
            case 2: mnLevelText = getMnName(2); if (roi2 > 1) tLuck = ((masternodeCoins/COIN) / roi2)*100; break;
            case 3: mnLevelText = getMnName(3); if (roi3 > 1) tLuck = ((masternodeCoins/COIN) / roi3)*100; break;
        }
    }
