...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid NSC address: ") + input.get_str());
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, addrList) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid NSC address: ")+name_);
